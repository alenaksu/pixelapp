...
gui.add(transform, 'edge', 0, 10, 0.1).onChange((v) => {
    instance.Sobel.size = v;
    update();
});
gui.add(transform, 'edgeThreshold', 0, 3, 0.05).onChange((v) => {
    instance.Sobel.threshold = v;
    update();
});
gui.add(transform, 'blur', 0, 10, 0.1).onChange((v) => {
    instance.Blur.size = v;
    update();
});
